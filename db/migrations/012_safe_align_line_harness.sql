-- Safe alignment: only add columns that don't exist yet
-- SQLite doesn't support IF NOT EXISTS for ALTER TABLE ADD COLUMN
-- So we use separate statements and let the migration runner handle errors

-- friends table: add missing columns (skip if already exists)
-- Each ALTER is a separate statement so one failure doesn't block others

-- Create all new LINE Harness tables (IF NOT EXISTS is safe)
